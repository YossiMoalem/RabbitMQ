...
class AMQPMessage : boost::noncopyable{
...
class AMQPBase : boost::noncopyable{
	protected:
		string name;
		short parms;
		amqp_connection_state_t * cnn;
		int channelNum;
		shared_ptr<AMQPMessage> pmessage;
...
class AMQPQueue : public AMQPBase  {
	protected:
		map< AMQPEvents_e, std::function<int( AMQPMessage* ) > > events;
	...
	public:
		...
		std::shared_ptr<AMQPMessage> getMessage() {
			return pmessage;
		}
